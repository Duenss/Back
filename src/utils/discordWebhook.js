...
const sendWebhook = async (webhookUrl, options, appId = null) => {
  ...
  // avatar_url solo funciona en Discord si es una URL directa a una imagen
  // (jpg/png/gif/webp). URLs de páginas web o servicios que redirigen son ignoradas.
  const isValidAvatarUrl = botAvatar &&
    typeof botAvatar === 'string' &&
    botAvatar.startsWith('http') &&
    /\.(png|jpg|jpeg|gif|webp)(\?.*)?$/i.test(botAvatar);

  const payload = {
    ...(botName ? { username: botName } : {}),
    ...(isValidAvatarUrl ? { avatar_url: botAvatar } : {}),
    embeds: [embed],
  };
